...
export default function GalleryForm({ initialData = null, isEditing = false }) {
  ...
  const [previewUrl, setPreviewUrl] = useState(initialData?.image_url || initialData?.url || null)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(error.error || `Failed to ${isEditing ? 'update' : 'save'} gallery entry`)
      }
    } catch (err) {
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          <h2 className="text-3xl font-black tracking-tight">{isEditing ? 'Edit Photo' : 'Add to Gallery'}</h2>
          <p className="text-primary font-bold text-sm uppercase tracking-widest">{isEditing ? 'Update the details' : 'Post a new memory'}</p>
        </div>
      </div>
                ...
                <span>
                  {uploading 
                    ...
                    : success 
                      ...
                      : (isEditing ? 'Save Changes' : 'Publish to Gallery')}
                </span>
              </button>
